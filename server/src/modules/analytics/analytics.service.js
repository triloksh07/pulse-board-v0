...
function buildQuestionAnalytics(poll, responses) {
  return poll.questions.map((question) => {
    const options = question.options.map((option) => {
      const count = responses.reduce((sum, response) => {
        const answer = response.answers.find((item) => asId(item.questionId) === asId(question._id));
        return sum + (answer?.selectedOptions.some((id) => asId(id) === asId(option._id)) ? 1 : 0);
      }, 0);
    ...
    const answeredCount = responses.filter((response) =>
      response.answers.some((answer) => asId(answer.questionId) === asId(question._id) && answer.selectedOptions.length)
    ).length;
    ...
    return {
      ...
      participationRate: responses.length ? Math.round((answeredCount / responses.length) * 100) : 0,
      options,
    };
  });
}
...
export async function getAnalyticsForPoll(pollId, userId = null, { publicOnly = false } = {}) {
  ...
  const responses = await Response.find({ pollId: poll._id }).sort({ submittedAt: 1 });
  const totalResponses = responses.length;
  const scores = responses.map((response) => response.score);
  const completionTimes = responses.map((response) => response.completionTime).filter(Boolean);
  const maxPossibleScore = poll.questions.reduce((sum, question) => sum + (question.points || 0), 0);
